    ...
    */
    ...
    $.fn.tableGroup = function (options) {
        ...
        return this.each(function () {
            ...
            var rowHeader, tHead;
            var NameGroupColumn;
            var editFunction = settings.editFunction;
            var deleteFunction = settings.deleteFunction;
                ...
                for (rowI = 0; rowI < body.rows.length; rowI++) {
                    ...
                    if (c) {
                        if (settings.useNumChars == 0) {
                            ...
                            if (settings.showInit != null && settings.showInit == false) {
                        ...
                        if (match !== lastIH) {
                            var editColumn = editFunction != null ? '<td align="right" style="width:25px"><img alt="Editar" src="Imagenes/edit.gif"  style="cursor:pointer;padding:3px" onclick="' + editFunction + '(this);return false;" /></td>' : "";
                            var deleteColumn = deleteFunction != null ? '<td align="right" style="width:25px"><img alt="Editar" src="Imagenes/delete.gif"  style="cursor:pointer;padding:3px" onclick="' + deleteFunction + '(this);return false;" /></td>' : "";
                            var tbodyName = "Tbody_" + NameBody;
                            var tablegroup = '<table cellpadding="0" cellspacing="0" width="100%" border="0"><tr><td onclick="return toggleGroup(' + nombreTable + ',grupo' + (NameBody + 1) + ',this);" class="rgGroupItemImageAbierto">&nbsp;</td><td class="rgGroupItem"><span>' + NameGroupColumn + ': ' + match + '</span></td>' + editColumn + deleteColumn +'</tr></table>';
                            //insert grouping row
...
function toggleGroup(Table, Group, img) {
    ...
    for (var i = 0; i < rowsGroup.length; i++) {
        if ($(rowsGroup[i]).css("display") == "" || $(rowsGroup[i]).css("display") == "block" || $(rowsGroup[i]).css("display") == "inline")
